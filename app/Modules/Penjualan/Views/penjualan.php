...
<template>
    ...
    <v-card>
        <v-card-title>
            <v-btn color="indigo" large dark href="<?= base_url('sales') ?>" elevation="1">
        ...
        <v-data-table :headers="headers" :items="data" :options.sync="options" :server-items-length="totalData" :items-per-page="10" :loading="loading" :search="search" class="elevation-1" loading-text="<?= lang('App.loadingWait'); ?>" dense>
            ...
            <template v-slot:item="{ item }">
                <tr>
                    ...
                    <td>{{Ribuan(item.bayar)}}</td>
                    <td>
                        {{RibuanLocale(item.kembali)}}
                        <v-btn icon small color="red" class="mr-3" link :href="'<?= base_url('piutang'); ?>?faktur=' + item.faktur" title="" alt="" v-if="item.id_piutang != null">
                            <v-icon small>mdi-book-arrow-left</v-icon>
                        </v-btn>
                    </td>
                    ...
                    <td>
                        ...
                        <?php } elseif ($cetakBluetooth == "1") { ?>
                            <v-btn icon class="mr-3" @click="printBT(item)">
                                <v-icon color="grey darken-3">mdi-printer</v-icon>
                            </v-btn>
                        <?php } else { ?>
                            <v-btn icon class="mr-3" @click="printUSB(item)">
                        ...
                        <!-- <v-btn icon class="mr-3" color="grey darken-3" :href="'<?= base_url('penjualan/printnota-pdf'); ?>' + '?id_penjualan=' + item.id_penjualan" target="_blank" title="Cetak PDF" alt="Cetak PDF">
                            <v-icon>mdi-file-pdf-box</v-icon>
                        </v-btn> -->

                        <v-btn icon class="mr-3" color="grey darken-3" :href="'<?= base_url('penjualan/printnota-html') ?>' + '?id_penjualan=' + item.id_penjualan" target="_blank" title="Print HTML" alt="Print HTML">
                            <v-icon color="grey darken-3">mdi-printer-outline</v-icon>
                        </v-btn>
            ...
            <v-card class="pa-2">
                ...
                <v-card-text>
                    ...
                    <div>
                        ...
                        Total: <span class="float-right">{{RibuanLocaleNoRp(total)}}</span><br />
