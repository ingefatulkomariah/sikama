    ...
    <v-card>
        <v-card-title>
            <v-btn color="indigo" dark @click="modalAddOpen" large elevation="1">
                <v-icon>mdi-account</v-icon> <?= lang('App.add') ?> User
            </v-btn>
            <v-spacer></v-spacer>
        ...
        <v-data-table :headers="tbUsers" :items="users" :items-per-page="10" :loading="loading" :search="search" class="elevation-1" loading-text="<?= lang('App.loadingWait'); ?>" dense>
            <template v-slot:item="{ item }">
                <tr>
                    ...
                    <td>
                        <span v-if="item.role == '1' || item.role == '3'">
                    ...
                    <td>
                        <v-btn icon color="primary" class="mr-3" @click="editItem(item)" title="Edit" alt="Edit">
                            <v-icon>mdi-pencil</v-icon>
                        </v-btn>
                        <v-btn icon color="indigo" @click="loginLog(item)" class="mr-3" title="Login Log" alt="Login Log">
                            <v-icon>mdi-clipboard-text-clock</v-icon>
                        </v-btn>
                        <v-btn icon color="warning" @click="changePassword(item)" class="mr-3" title="" alt="">
            ...
            <v-card>
                ...
                <v-card-text class="py-5">
                    <v-form v-model="valid" ref="form">
                        ...
                        <v-select v-model="role" name="role" :items="roles" item-text="label" item-value="value" label="Select Role *" :error-messages="roleError" outlined>
                    </v-form>
                </v-card-text>
                <v-divider></v-divider>
                <v-card-actions>
