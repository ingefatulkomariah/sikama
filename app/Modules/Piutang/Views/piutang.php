...
<template>
    ...
    <v-card>
        <v-card-title>
            ...
            <v-select v-model="status" label="Filter Status" :items="dataStatus" item-text="text" item-value="value" attach multiple hide-details style="max-width: 300px;"></v-select>
            <v-spacer></v-spacer>
...
<script>
    ...
    methodsVue = {
        ...
        getPiutangStatus: function() {
            this.loading = true;
            axios.get(`<?= base_url() ?>api/piutang?tgl_start=${this.startDate}&tgl_end=${this.endDate}&status=${this.status}`, options)
                .then(res => {
                    // handle success
                    this.loading = false;
                    var data = res.data;
